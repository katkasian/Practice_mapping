...
DS = 'ua_nanmin_2'  # change the name of the dataset here
TB = 'tweets'
LIM = 100000000  # change the number of the items needed
CLST_TB = 'nanmin_rt_mod1_deg3'  # change the name of modularity table here
CLST_EXCL = ['6', '11']
...
def load_data_from_bq(dataset, table, cluster_table, clusters_to_exclude, limit):
    ...
    query_string = f"""
CREATE TABLE IF NOT EXISTS {dataset}.gpt_labeled_practices (tweet_id STRING, practice STRING);

WITH already_labeled AS (
  SELECT tweet_id 
  FROM {dataset}.gpt_labeled_practices
...
eligible_tweets AS (
  ...
  LEFT JOIN {dataset}.{cluster_table} clust 
      ON twt.author_username = clust.node_id 
  LEFT JOIN {dataset}.{cluster_table} ref_clust 
      ON twt.referenced_tweet_author_username = ref_clust.node_id 
  LEFT JOIN already_labeled al 
      ON CAST(twt.tweet_id AS STRING) = al.tweet_id
  WHERE clust.modularity_class NOT IN UNNEST({clusters_to_exclude})
    AND (ref_clust.modularity_class IS NULL 
         OR ref_clust.modularity_class NOT IN UNNEST({clusters_to_exclude}))
    AND tweet_type = 'original'
    and CONTAINS_SUBSTR(tweet_text, '難民')
...
def clean_data_from_bq(data):
    ...
    return data.head(20) #making sure prompt is not too long


def prepare_openai_prompt(twt_data):
    with open('mar_2024_prompts/v6_cot/Ref_MPE_COT_0_with_system_instruction.txt', encoding='utf-8') as f:
...
def save_open_ai_response(merged_df):
    now = datetime.now()
    merged_df.to_csv(f'full_datasets_labelled_backup/ref/labeled_{DS}_{now.strftime("%Y_%m_%d_%H_%M_%S")}.csv')
...
def push_labeled_data_to_bq(merged_labeled_df):
    ...
    to_push['tweet_id'] = to_push.tweet_id.apply(str)
