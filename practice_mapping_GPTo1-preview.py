...
def call_openai_chat(model, text):
    try:
        response = client.chat.completions.create(
            model=model,
            messages=[
                {'role': 'user', 'content': '''Assistant is a large language model trained by OpenAI.
                Instructions: 
                ...
                -In your response, ONLY use the name of the practice and do not explain any reasoning. Return only one name from this list: [Advocacy, Arguing, Audiencing, Boosting, Community work, Expressing solidarity, Fundraising, Knowledge performance, Membership requests, Meme creation, Mobilising, News and content curation, Play, Self-promotion, Shitposting, Not applicable]
                '''},
        ...
        #need a better exception handling here - we want the results saved
...
if __name__ == '__main__':
    ...
    for data in validation_dataset.iterrows():
        ...
        result = call_openai_chat("o1-preview", prompt_text)
