...
DS = 'ua_eurovision_2023'  # change the name of the dataset here
TB = 'tweets'
LIM = 10000  # change the number of the items needed
CLST_TB = 'degree2_mod_05_clusters'  # change the name of modularity table here
CLST_EXCL = ['196', '191']
...
def clean_data_from_bq(data):
    ...
    return data.head(31) #making sure prompt is not too long
...
def save_open_ai_response(merged_df):
    now = datetime.now()
    merged_df.to_csv(f'full_datasets_labelled_backup/esc/labeled_{DS}_{now.strftime("%Y_%m_%d_%H_%M_%S")}.csv')
